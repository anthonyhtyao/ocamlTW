...
INSERT INTO article (id,category,title,content,slg,previous,next)

VALUES

  (1, 1, 'Préparez-vous', 'Start to learn OCaml! 開始建立環境。',
  'prepare-yourself', null, 2),
  (2, 1, 'Why OCaml?', 'the article that explains the advantages
   of OCaml here', 'why-ocaml',1,3),
  (3, 1, 'How to install OCaml?', 'sudo apt-get install ocaml',
  'how-to-install-ocaml',2,4),
  (4, 1, 'Compile OCaml', 'ocamlc -o meow meow.ml', 'compile-ocaml',3,5),
  (5, 1, 'Hello World!', 'print_string "Hello World!"', 'hello-world',4,6),

  (6, 2, 'OCaml 入門', '從最核心的部份談起，包含基本的四則
   運算、函數定義、以及型別的概念等等', 'ocaml-core-language',5,7),
  (7, 2, '基本資料型別與運算子什麼的名稱實在太土了我絕對不想用',
  '從現在開始就正式進入我們 tuto 的第一堂課了，當然是從基礎中的基礎基本變數
   宣告、物件型別還有運算子開始講起囉！', 
  'declarations-operations-basic-types',6,8),
  (8, 2, '函數',
  '你 C 和 C++ 駕輕就熟同時也是 Java 能手你相信在第一節結束之後我們肯定是要
   講流程控制 if else, for 還有 while，然而竟然是函數！',
  'functions',7,9),
  (9, 2, '型別', 'Let''s talk about types.', 'typing',8,10),
  (10, 2, 'Let''s write a tree', 
  'type tree = F of int | T of tree*tree','let''s-write-a-tree',9,11),
  (11, 2, 'Imperative features', 'for, ref, while, if else, mutable, array',
  'imperative-features',10,12),
  (12,2,'例外處理','let x = try 1/0 with _ -> 10', 'exception',11,13),
  (13,2,'Pretty printing',
  'Format.printf "@[an integer: %d and a string: %s@]@?" 5 "hehe"',
  'pretty-printing',12,14),
  (14,2,'來寫些簡單的圖論演算法', 
  'Dijkstra, Floyd-Warshall, Kruskal, blosso, Ford-Fulkerson',
  'algorithms-for-graphs',13,15),
  
  (15,3,'OCaml 的 Module 系統','哈哈我都不會反正要講 Module', 'module',14,16),
  (16,3,'Module 基本', 'module M = struct let c = 100 let f x = c * x end', 
  'structure-signature',15,17),
  (17,3,'Functor','Fuctor 很厲害','functor',16,18),
  (18,3,'Seperate compilation','這是啥所以','seperate-compilation',17,null);
